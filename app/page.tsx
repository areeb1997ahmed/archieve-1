...
export default function SpeculativeArchive() {
  ...
  const sections = [
    { id: "story", title: "My Story", component: <MyStory /> },
  ...
  const getNavName = (data: any) => {
    setNav(data)
  }
  return (

    <>
      <PageLoader isLoading={isLoading} duration={4000} />
      ...
      {isLoading == false && (
        <>
          {hide == false ? (
            <>
              <TextReveal
                children="A painting requires a little mystery, some vagueness, and some fantasy. When you always make your meaning perfectly plain you end up boring people"
                func={hideFunction}
              />
              <div
                className="fixed bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center space-y-2 transition-opacity duration-300 z-50 opacity-100" >
                <span className="text-sm text-gold-500 dark:text-gold-400">Scroll Down</span>
                <ChevronDown className="h-6 w-6 text-gold-500 dark:text-gold-400 animate-bounce" />
              </div>
            </>
          ) : null}
          ...
          <div className=" bg-gradient-premium relative overflow-hidden">
            ...
            {/* Navigation Progress */}
            <NavigationProgress sections={sections} getNavName={getNavName} currentSection={currentSection} />
